...
describe.sequential("publish packaging", () => {
  it("rewrites workspace package versions for canary publishing", async () => {
    ...
    try {
      ...
      await writeFile(
        ...
        `${JSON.stringify({ name: "@actalk/quire-core", version: "0.4.6" }, null, 2)}\n`,
      );
      await writeFile(
      ...
      expect(cliPackageJson.dependencies["@actalk/quire-core"]).toBe("0.4.8-canary.7");
    } finally {
  ...
  it("links internal source dependencies through the workspace", async () => {
    ...
    expect(cliPackageJson.dependencies["@actalk/quire-core"]).toBe("workspace:*");
    expect(cliPackageJson.dependencies["@actalk/quire-studio"]).toBe("workspace:*");
    expect(studioPackageJson.dependencies["@actalk/quire-core"]).toBe("workspace:*");
  });
  ...
  it("accepts canonical workspace protocol in source manifests", async () => {
    ...
    try {
      ...
      await writeFile(
        join(tempCoreDir, "package.json"),
        `${JSON.stringify({ name: "@actalk/quire-core", version: "0.5.1" }, null, 2)}\n`,
      );
      await writeFile(
        join(tempCliDir, "package.json"),
        `${JSON.stringify(
        ...
        `${JSON.stringify(
          {
            ...
            version: "0.5.1",
            dependencies: {
              "@actalk/quire-core": "workspace:0.5.0",
            },
          },
          null,
          2,
  ...
  it("replaces workspace dependencies before npm pack", { timeout: CLI_PACK_TEST_TIMEOUT_MS }, async () => {
    ...
    try {
      ...
      expect(packedPackageJson.dependencies["@actalk/quire-core"]).toBe(corePackageJson.version);
      expect(packedPackageJson.dependencies["@actalk/quire-studio"]).toBe(studioPackageJson.version);
    } finally {
  ...
  it("packs the built-in professional skills with core", { timeout: STUDIO_PACK_TEST_TIMEOUT_MS }, async () => {
    ...
    try {
      ...
      expect(archiveListing).toContain("package/skills/quire-long-writing/SKILL.md");
      expect(archiveListing).toContain("package/skills/quire-story-review/references/review-matrix.md");
      expect(archiveListing).toContain("package/skills/quire-story-cover/SKILL.md");
    } finally {
      await rm(packDir, { recursive: true, force: true });
    }
  });
