...
vi.mock("@actalk/quire-core", async (importOriginal) => {
  ...
  return {
    ...
    // The pipeline, the job queue and the storybook stages, real rather than
    // stubbed. They are file-and-state code over the test's own temp root, so a
    // stub could only ever prove the stub works — and leaving them out is not
    // neutral: the server calls one of them at boot, so a missing name here
    // fails every test in the file with an error about a mock.
    advancePipeline: actual.advancePipeline,
    runStage: actual.runStage,
    allPipelineRuns: actual.allPipelineRuns,
    markPipelineInterrupted: actual.markPipelineInterrupted,
    resumePipeline: actual.resumePipeline,
    pausePipeline: actual.pausePipeline,
    registerExecutor: actual.registerExecutor,
    approvePipelineGate: actual.approvePipelineGate,
    rejectPipelineGate: actual.rejectPipelineGate,
    withdrawPipelineGate: actual.withdrawPipelineGate,
    ensurePipeline: actual.ensurePipeline,
    loadPipeline: actual.loadPipeline,
    pipelineFor: actual.pipelineFor,
    pipelineWaitingOn: actual.pipelineWaitingOn,
    stageSequence: actual.stageSequence,
    reportUnitDone: actual.reportUnitDone,
    refFromPath: actual.refFromPath,
    ...
    createStorybook: actual.createStorybook,
    planStorybook: actual.planStorybook,
    writeStorybookSpread: actual.writeStorybookSpread,
    approvePublication: actual.approvePublication,
    approvePublicationDesign: actual.approvePublicationDesign,
    unapprovePublication: actual.unapprovePublication,
    unapprovePublicationDesign: actual.unapprovePublicationDesign,
    openPublicationIssue: actual.openPublicationIssue,
...
describe("createStudioServer daemon lifecycle", () => {
  ...
  it("classifies Quire parser/tool errors as internal instead of blaming the selected provider", async () => {
    ...
    expect(json.error.message).toContain("Quire 内部流程错误");
